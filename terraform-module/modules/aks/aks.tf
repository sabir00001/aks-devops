...
resource "azurerm_kubernetes_cluster" "aks_cluster" {
  location            = var.location                                            # Location where the AKS cluster will be deployed
  name                = "${var.cluster_name}-cluster-${var.environment}"    # Name of the AKS cluster
  resource_group_name = var.rg_name                                            # Name of the Azure resource group where the AKS cluster will be deployed
  ...
  default_node_pool {
    ...
    os_disk_size_gb     = 30
    vnet_subnet_id      = var.aks_subnet_id #ID of the subnet in the VNet
